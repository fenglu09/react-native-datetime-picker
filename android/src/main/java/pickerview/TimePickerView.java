    ...
    public void onClick(View v) {
        ...
        if (tag.equals(TAG_SUBMIT)) {
            ...
            try {
                ...
                if (Pickerview.Datetype.equals("datetime")) {
                ...
                if (Pickerview.minDate != null) {
                    if (date.before(Pickerview.minDate)) {
                        String tip = sdf.format(Pickerview.minDate);
                        Toast.makeText(Pickerview.context, "选择日期不能小于" + tip, 0).show();
                        return;
                    }
                }
                if (Pickerview.maxDate != null) {
                    if (date.after(Pickerview.maxDate)) {
                        String tip = sdf.format(Pickerview.maxDate);
                        Toast.makeText(Pickerview.context, "选择日期不能大于" + tip, 0).show();
                        return;
                    }
                }
                returnData();
